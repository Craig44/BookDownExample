...
Create a folder in your github repository that contains your bookdown. A good way to get started is to download the GitHub repository <https://github.com/rstudio/bookdown-demo> as a Zip file, then unzip it locally. This is a minimal bookdown that is very helpful to get started.


Once you have a bookdown that compiles locally commit and push the necessary files to the remote repository and create a github actions workflow. I use the following yaml script can be [found here](https://github.com/Craig44/BookDownExample/blob/main/.github/workflows/deploy_bookdown.yml). Which we will work through soon. Before we do that, the next thing you will need to do is setup a github.io for your username or organisation [see here](https://pages.github.com/) for more information. The next thing you will need is to create an empty branch on your github repository to deploy the bookdown. I used the git command line calls below which was taken from [here](https://gist.github.com/ramnathv/2227408)
...
```
the parameters and inputs for this github action may change over time, to see the latest options and version [see here](https://github.com/Cecilapp/GitHub-Pages-deploy). THe key thing in this set up is makeing sure your repository allows read and write functions for the workflow. To see and change these go to settings tab of your github repository followed by the Actions tab scroll down to the `Workflow permissions` section where you want to make sure it is set to read and write.
